...
<html>
  ...
  <body>
  	<div class="boxi" >
   	...
   	<c:if test="${strlike eq 'like'}">
   		<div class="page">
   			当前页:[${page.now }/${page.count }]&nbsp;&nbsp;
   			<c:if test="${page.now>1 }">
   				<a href="../../SelectBookInfoLike?index=${page.first }&bname=${bname}" target="mainFrame">首页</a>&nbsp;&nbsp;
   				<a href="../../SelectBookInfoLike?index=${page.back }&bname=${bname}" target="mainFrame">上一页</a>&nbsp;&nbsp;
   			</c:if>
   			<c:if test="${page.now<page.count }">
   				<a href="../../SelectBookInfoLike?index=${page.next }&bname=${bname}" target="mainFrame">下一页</a>&nbsp;&nbsp;
   				<a href="../../SelectBookInfoLike?index=${page.last }&bname=${bname}" target="mainFrame">未页</a>&nbsp;&nbsp;
   			</c:if>
   		</div>
   	</c:if>
   	
   	 <c:if test="${strlike eq 'null' }">
